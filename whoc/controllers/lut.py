"""
LUT controller class. Given wind speed components ux and uy for some future prediction horizon at each wind turbine,
Output yaw angles equal to those oprimized for that wind magnitude and direction offline.
...
from ..simulators import FlorisSimulator
...
if __name__ == "__main__":
    ...
    fi_bl = FlorisSimulator().load_floris(
        config_path='/Users/aoifework/Documents/toolboxes/floris/examples/inputs/gch.yaml')
    fi_bl.env.reinitialize(
    ...
    farm_power_bl = fi_bl.env.get_farm_power()
    ...
    fi_opt = FlorisSimulator().load_floris(
        config_path='/Users/aoifework/Documents/toolboxes/floris/examples/inputs/gch.yaml')

    fi_opt.env.reinitialize(
    ...
    # Pour this into a parallel computing interface
    fi_opt.parallelize(max_workers)
    
    # Now optimize the yaw angles using the Serial Refine method
    df_opt = fi_opt.env.optimize_yaw_angles(
    ...
    farm_power_opt = fi_bl.env.get_farm_power(yaw_angles=yaw_grid)
...
class LUTController(ControllerBase):
    def __init__(self, interface):
        ...
        # TODO optimize, unless passed existing lookup table
	def solve(self):
        pass
    
    def plot_power_vs_speed(self):
    ...
    def plot_yaw_vs_winddir(self):
        ...
        plt.show()
    def plot_power_vs_dir(self):
        # Plot power and AEP uplift across wind direction
        wd_step = np.diff(fi_bl.floris.flow_field.wind_directions)[0]  # Useful variable for plotting
        fig, ax = plt.subplots(nrows=3, sharex=True)
