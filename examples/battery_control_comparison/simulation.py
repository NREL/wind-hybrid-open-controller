...
reference_input_sequence = np.concatenate((
    20000*np.ones(2*hour),
    -20000*np.ones(3*hour),
    10000*np.ones(1*hour),
    0*np.ones(1*hour),
    -10000*np.ones(1*hour),
    0*np.ones(1*hour),
    np.repeat(0+5000*np.random.randn(round(hour/(12*60))), (12*60)),
))


# Create some functions for simulating for simplicity
def simulate(k_p_max, k_p_min):
...
def plot_results(time, power_sequence, soc_sequence):
    # Plot
    ...
    ax[0].grid()
    ax[1].grid()
    ax[0].set_title("k_p_max = {:.3f}, k_p_min = {:.3f}".format(k_p_max, k_p_min))
    ax[0].plot([time[0], time[-1]], [20000, 20000], color="red", linestyle="dotted")
    ax[0].plot([time[0], time[-1]], [-20000, -20000], color="red", linestyle="dotted")

# Loop over different gains
k_p_max_vals = [1.0, 0.01, 1.0]
k_p_min_vals = [1.0, 0.01, 0.01]

# Run simulations and plot
for k_p_max, k_p_min in zip(k_p_max_vals, k_p_min_vals):
    time, power_sequence, soc_sequence = simulate(k_p_max, k_p_min)
    plot_results(time/3600, power_sequence, soc_sequence)
