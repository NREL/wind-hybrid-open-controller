...
powers_wind_only = df_wind[pow_cols].to_numpy()
powers_with_batt = df_batt[pow_cols].to_numpy()
...
ax[0].plot(time/3600, df_wind[ws_col], color="C0", label="Wind speed")
ax[0].set_ylabel("Wind speed [m/s]")
ax[0].grid()

# Power output wind only
ax[1].plot(time/3600, powers_base.sum(axis=1), color="black", label="Available wind power", alpha=0.5)
ax[1].fill_between(time/3600, powers_wind_only[:,0], label="Wind power")
ax[1].plot(time/3600, powers_wind_only.sum(axis=1), color="black", label="Plant power")
ax[1].plot(time/3600, flexible_interconnect, color="red", linestyle="dotted", label="Flexible interconnect limit")

# Power output wind + battery
ax[2].plot(time/3600, powers_base.sum(axis=1), color="black", label="Available wind power", alpha=0.5)
ax[2].fill_between(time/3600, battery_power, color="C1", label="Battery power")
ax[2].fill_between(time/3600, battery_power, powers_with_batt[:,0]+battery_power, color="C0", label="Wind power")
ax[2].plot(time/3600, powers_with_batt.sum(axis=1)+battery_power, color="black", label="Plant power")
ax[2].plot(time/3600, flexible_interconnect, color="red", linestyle="dotted", label="Flexible interconnect limit")
...
curtailed_hrs = ((powers_base - powers_wind_only).sum(axis=1) > 10).sum() * dt/3600
