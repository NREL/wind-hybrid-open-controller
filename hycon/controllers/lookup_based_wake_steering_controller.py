...
class LookupBasedWakeSteeringController(ControllerBase):
    ...
    def set_controller_parameters(self, df_yaw=None, hysteresis_dict=None, yaw_IC=270.0):
        if df_yaw is None:
            if hysteresis_dict is not None:
                raise ValueError(
                    "Hysteresis zones provided without yaw offsets. Please provide yaw offsets."
                )
            if self.verbose:
        ...
        # For startup
        self.wd_store = [270.0] * self.n_turbines  # TODO: update this?
    ...
    def wake_steering_angles(self, wind_directions):
        ...
        if self.wake_steering_interpolant is None:
            ...
            interpolated_angles = self.wake_steering_interpolant(wind_directions, wind_speeds, None)
        ...
        if self.hysteresis_dict is not None:
            for t in range(self.n_turbines):
                for zone in self.hysteresis_dict["T{:03d}".format(t)]:
                    if (zone[0] < wind_directions[t] < zone[1]) or (
                        wrap_180(zone[0]) < wrap_180(wind_directions[t]) < wrap_180(zone[1])
                    ):
