...
def test_apply_static_rate_limits():
    ...
    ti_resolution = 0.01
    df_opt = generic_df_opt(
        wd_resolution=wd_resolution, ws_resolution=ws_resolution, ti_resolution=ti_resolution
    )
...
def test_apply_wind_speed_ramps():
    ...
    assert np.allclose(
        np.vstack(
            df_opt_ramps[df_opt_ramps.wind_speed >= ws_wake_steering_cut_out].yaw_angles_opt.values
        ),
        0.0,
    )
    ...
    assert np.allclose(
        ...
        (
            np.vstack(
                df_opt_ramps[
                    df_opt_ramps.wind_speed == ws_wake_steering_cut_out
                ].yaw_angles_opt.values
            )
            + np.vstack(
                df_opt_ramps[
                    df_opt_ramps.wind_speed == ws_wake_steering_fully_engaged_high
                ].yaw_angles_opt.values
            )
        )
        / 2,
...
def test_hysteresis_zones():
    ...
    assert (
        (np.array(hysteresis_dict_test["T000"][0]) - 90.0) % 360.0
        == np.array(hysteresis_dict_base["T000"][0])
    ).all()
    ...
    assert (
        hysteresis_dict_test["T000"][0][1] - hysteresis_dict_test["T000"][0][0] > 3 * min_zone_width
    )
    ...
    df_opt_2 = df_opt_2.sort_values(by=["wind_direction", "wind_speed", "turbulence_intensity"])
    hysteresis_dict_test = compute_hysteresis_zones(
        df_opt_2, min_zone_width=3 * min_zone_width, yaw_rate_threshold=1.0, verbose=True
    )
    # Check actual grouping occurs (not purely due to larger region width)
    assert (
        hysteresis_dict_test["T000"][0][1] - hysteresis_dict_test["T000"][0][0]
    ) % 360.0 > 3 * min_zone_width
    # Check new region covers original region
...
def test_consolidate_hysteresis_zones():
    ...
    # Larger set
    width = 6.0
    hysteresis_centers = [8.0, 12.0, 16.0, 344.0, 348.0, 352.0, 359.0]
